...
export function usePortrait() {
  ...
  const fetchRandomArtwork = async (gender: string = ""): Promise<ArtworkData | null> => {
    try {
      ...
      // Use POST request with Elasticsearch Query DSL for multiple filters
      ...
      // Add gender filter if specified (using text search instead of subject_ids)
      if (gender) {
        requestBody.q = `portrait ${gender}`;
      }
